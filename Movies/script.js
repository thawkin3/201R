...
function runAJAX(theResource) {
	...
	$.getJSON("http://api.themoviedb.org/3/movie/" + theResource + "?api_key=905e94f60d94ae753e3aa1126d23d0f2").done(function(data) {
		...
		$("#container").append(
			"<table>" + 
				"<thead>" + 
					"<tr style='background: #6F8DC3;'>" + 
						"<th id='poster'>Poster</th>" + 
						"<th class='sortable' id='title'>Title<img class='arrow activeSort' src='up.png' /></th>" + 
						"<th class='sortable' id='rating'>Rating<img class='arrow' src='up.png' /></th>" + 
						"<th class='sortable' id='popularity'>Popularity<img class='arrow' src='up.png' /></th>" + 
		...
		for (var i = 0; i < movieArr.length; i++) {
			$("table tbody").append(
				"<tr>" + 
					"<td><img class='pics' src ='" + movieArr[i].poster + "' /></td>" + 
					"<td>" + movieArr[i].title + "</td>" + 
					"<td>" + movieArr[i].rating + "</td>" + 
					"<td>" + parseFloat(movieArr[i].popularity).toFixed(2) + "</td>" + 
					"<td style='max-width:300px'>" + movieArr[i].overview + "</td>" + 
				"</tr>"
			);
		}
	...
	if ($(this).attr("id") != "poster" && $(this).attr("id") != "overview") {
		...
		for (var i = 0; i < movieArr.length; i++) {
			$("table tbody").append(
				"<tr>" + 
					"<td><img class='pics' src='" + movieArr[i].poster + "' /></td>" + 
					"<td>" + movieArr[i].title + "</td><td>" + movieArr[i].rating + "</td>" + 
					"<td>" + parseFloat(movieArr[i].popularity).toFixed(2) + "</td>" + 
					"<td style='max-width:300px'>" + movieArr[i].overview + "</td>" + 
				"</tr>"
			);
